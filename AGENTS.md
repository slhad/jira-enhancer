...
pnpm test           # Run all unit tests (Vitest, 76 tests across 8 files)
...
- State is managed by `useRefinement` hook. Components receive state as props.
- Chrome API calls belong in hooks (`useNativeMessaging`, `useRefinement`), not in components.
- Do not call `chrome.*` APIs outside of hooks or the background service worker.
...
- Do not create files outside the three packages and `plans/` unless there is a clear structural reason.
- Do not create `*.md` documentation files for individual changes — update `README.md` or this file instead.
